...
export const Route = createFileRoute("/waitlist")({
  head: () => ({
    meta: [
      { title: "Join the Waitlist — OxVerse Academy" },
      { name: "description", content: "Reserve your seat for the next OxVerse cohort. Be the first to know when applications open and get priority admission." },
      { property: "og:title", content: "Join the OxVerse Waitlist" },
      { property: "og:description", content: "Priority access to Africa's most ambitious tech academy." },
    ],
  }),
        ...
        <section className="mx-auto max-w-3xl px-6 pt-32 pb-24">
          ...
          >
            ...
            <p className="mt-4 text-primary-foreground/85 text-pretty">
              You're <strong>#{success.position}</strong> on the waitlist for <strong>{success.courseTitle}</strong>. We'll email <strong>{success.email}</strong> the moment applications open and give you priority admission.
            </p>
          ...
          <div className="mt-8 grid sm:grid-cols-2 gap-4">
            <a
              href={whatsappLink(`Hi OxVerse, I just joined the waitlist for ${success.courseTitle}. My name is ${success.fullName}.`)}
          ...
          <button
            onClick={() => { setSuccess(null); setForm({ fullName: "", email: "", phone: "", courseSlug: "", experience: "None", reason: "" }); }}
      ...
      <section className="relative">
        ...
        <div className="relative mx-auto max-w-7xl px-6 pt-24 pb-10">
          ...
          <p className="mt-6 max-w-2xl text-lg text-ink-muted text-pretty">
            Cohorts fill up fast. Reserve your spot and we'll notify you the moment applications open — with priority admission for waitlist members.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 grid lg:grid-cols-12 gap-10">
          ...
          <form onSubmit={onSubmit} className="rounded-3xl border border-border bg-background p-8 lg:p-10 space-y-5">
            ...
            <Field label="Course you're interested in" error={errors.courseSlug}>
              ...
              >
                ...
                {courses.map((c) => (
                  <option key={c.slug} value={c.slug}>{c.title} — {c.duration}</option>
                ))}
              </select>
            </Field>
                ...
                {(["None", "Beginner", "Intermediate", "Advanced"] as const).map((lvl) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-medium border transition ${
                      form.experience === lvl ? "bg-ink text-background border-ink" : "border-border text-ink-muted hover:border-ink hover:text-ink"
                    }`}
                  >
                    {lvl}
                  </button>
            ...
            <Field label="Why do you want to join? (optional)" error={errors.reason}>
              <textarea
                ...
                placeholder="Share your goals — it helps us prepare your onboarding."
                maxLength={500}
              />
            </Field>
            ...
            <p className="text-xs text-ink-muted text-center">
              By joining, you agree to receive admission updates. We never share your details.
            </p>
          </form>
        </div>
        ...
        <aside className="lg:col-span-5 space-y-4">
          ...
          <div className="rounded-3xl border border-border p-6 bg-background">
            ...
            <p className="mt-2 text-sm text-ink-muted">Across all upcoming cohorts.</p>
          </div>

          <a
            href={whatsappLink("Hi OxVerse, I'd like to join the waitlist.")}
            target="_blank" rel="noreferrer"
            className="block rounded-3xl border border-border p-6 bg-background hover:border-primary transition"
          >
            <p className="text-sm text-ink-muted">Prefer a quick chat?</p>
            <p className="font-display text-xl font-semibold mt-1">WhatsApp admissions</p>
            <p className="text-sm font-medium text-primary mt-1">{SITE.phoneDisplay}</p>
          </a>
        </aside>
      </section>
    </SiteLayout>
