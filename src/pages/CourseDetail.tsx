...
const sampleTestimonials = [
  ...
  },
  {
    name: "Amara Nwosu",
    role: "OxVerse Alum",
    quote: "Mentors who actually care, classmates who push you, and a campus built for deep work.",
  },
  {
    name: "Kola Ibrahim",
    role: "OxVerse Alum",
    quote:
      "From beginner to confident professional in months. Best decision I've made for my career.",
  },
];
...
export default function CourseDetailPage() {
  ...
  const fallbackRelated =
    related.length === 0 ? courses.filter((c) => c.slug !== course.slug).slice(0, 3) : related;
          ...
          <div className="mt-6 grid lg:grid-cols-12 gap-10 items-start">
            <div className="lg:col-span-7">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Link
                  to="/contact"
                  className="inline-flex items-center gap-2 rounded-full border border-border bg-background px-7 py-4 font-semibold hover:border-ink transition"
                >
                  Talk to admissions
                </Link>
              </div>
        ...
        >
          ...
          {tab === "Curriculum" && (
            <div className="space-y-6 max-w-4xl">
              ...
              <div className="rounded-3xl border border-primary/30 bg-primary/5 p-6 text-sm text-ink-muted">
                <p className="font-semibold text-primary">
                  Full curriculum access is included with enrollment.
                </p>
                <p className="mt-2">
                  Register to unlock the complete course curriculum, weekly modules, and bonus
                  resources.
                </p>
              </div>
            </div>
          ...
          {tab === "Projects" && (
            <div className="grid sm:grid-cols-2 gap-6 max-w-5xl">
              <h2 className="sm:col-span-2 font-display text-3xl font-bold tracking-tight">
                Projects you'll build
              </h2>
              {course.projects.map((p, i) => (
                <div
                  key={i}
                  className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-100 to-purple-300 p-8 aspect-video flex items-end"
                >
            ...
            <div className="max-w-3xl">
              ...
              <ul className="mt-6 space-y-3">
                {course.requirements.map((r, i) => (
                  <li key={i} className="flex gap-3 rounded-2xl border border-border p-5">
                    <Check className="size-5 text-primary shrink-0 mt-0.5" /> {r}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {tab === "Schedule" && (
            <div className="max-w-3xl">
              ...
              <p className="mt-3 text-ink-muted">
                Choose the cadence that fits your life. Both tracks cover the same material.
              </p>
              <div className="mt-6 space-y-3">
                {course.schedule.map((s, i) => (
                  <div
                    key={i}
                    className="rounded-2xl border border-border p-6 flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3">
          ...
          {tab === "Career Path" && (
            <div className="max-w-4xl">
              ...
              <div className="mt-6 grid sm:grid-cols-2 gap-3">
                {course.career.map((c) => (
                  <div key={c} className="rounded-xl border border-border px-5 py-4 font-medium">
                    {c}
                  </div>
                ))}
              </div>
          ...
          {fallbackRelated.map((c) => (
            ...
            >
              <div className="relative aspect-[16/11] overflow-hidden">
                ...
                />
                <span className="absolute top-3 left-3 inline-flex items-center rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-primary">
                  {c.category}
                </span>
              </div>
              <div className="p-5">
                <p className="font-display text-lg font-semibold group-hover:text-primary transition">
                  {c.title}
                </p>
      ...
      <section className="border-t border-border bg-ink text-background">
        <div className="mx-auto max-w-5xl px-6 py-20 text-center">
          ...
          <p className="mt-4 text-background/70 max-w-xl mx-auto">
            Reserve your seat in the next cohort. Limited to 25 students.
          </p>
          <Link
            to={`/waitlist`}
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-background text-foreground px-7 py-4 font-semibold hover:bg-primary hover:text-primary-foreground transition"
          >
            Enroll Now <ArrowRight className="size-4" />
          </Link>
        </div>
