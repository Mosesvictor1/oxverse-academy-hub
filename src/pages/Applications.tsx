    ...
    <SiteLayout>
      ...
      <section className="relative">
        ...
        <div className={`relative ${pageHeroWideClass}`}>
          ...
          <h1 className="mt-3 font-display text-3xl sm:text-4xl md:text-6xl font-bold tracking-tighter">My applications</h1>
          <p className="mt-3 text-ink-muted text-base sm:text-lg max-w-2xl text-pretty">Track each application you've submitted and its current admission status.</p>
        </div>
      </section>

      {justSubmitted && (
        <section className={`${pageSectionWideClass} pt-0 pb-6 sm:pb-8`}>
          <SuccessCard eyebrow="Application received" title="Application received" icon={CheckCircle2}>
            <SuccessParagraph>
        ...
        {apps.length === 0 ? (
          ...
          <div className="space-y-4">
            {apps.map((a) => {
              ...
              return (
                <div key={a.id} className="rounded-2xl border border-border p-4 sm:p-6 hover:shadow-lg transition">
