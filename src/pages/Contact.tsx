...
export default function ContactPage() {
  ...
  return (
    <SiteLayout>
      ...
      <section className={`${pageHeroClass} pb-6 sm:pb-10`}>
        <SectionEyebrow>Contact</SectionEyebrow>
        <h1 className="mt-6 font-display text-4xl sm:text-5xl md:text-6xl font-bold tracking-tighter">Visit. Call. Write.</h1>
      </section>
      <section className={`${pageSectionClass} grid lg:grid-cols-2 gap-8 sm:gap-12 pt-0`}>
        <div className="space-y-4 sm:space-y-6">
          {[
            ...
            <div key={c.t} className="rounded-2xl border border-border p-4 sm:p-6 flex items-start gap-4">
              <c.i className="size-6 text-primary mt-1 shrink-0" />
              <div className="min-w-0">
                <p className="text-sm text-ink-muted">{c.t}</p>
                <p className="font-display text-lg sm:text-xl font-semibold mt-1 break-words">{c.v}</p>
              </div>
            </div>
          ))}
          <a
            ...
            className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-full bg-primary text-primary-foreground px-6 py-3 font-semibold hover:opacity-90 transition"
          >
            <MessageCircle className="size-4" /> Chat on WhatsApp
          </a>
        </div>
        {sent ? (
          <SuccessCard eyebrow="Message sent" title="Thanks, we'll be in touch.">
            <SuccessParagraph>
              A member of admissions will reply within 24 hours.
            </SuccessParagraph>
          </SuccessCard>
        ) : (
