...
import { FormAside } from "@/components/site/BenefitsPanel";
...
const tiers = [
  { range: "1 to 10 referrals", reward: "5% commission" },
  { range: "11 to 25 referrals", reward: "7% commission" },
  { range: "26 to 50 referrals", reward: "10% commission" },
...
export default function AmbassadorPage() {
  ...
  if (success) {
    ...
    return (
      <SiteLayout>
        ...
        <SuccessScreen>
          <SuccessCard eyebrow="Application received" title={`You're in, ${firstName}!`}>
            <SuccessParagraph>
              Your ambassador application has been received. We&apos;ll review and email{" "}
              <strong className="font-semibold break-all">{displayEmail}</strong> with your unique
              referral code/link and the ambassador starter pack.
            </SuccessParagraph>
            <SuccessDetail label="Category">{success.category}</SuccessDetail>
          </SuccessCard>
  ...
  return (
    <SiteLayout>
      ...
      <section className={`${pageSectionClass} grid md:grid-cols-3 gap-4 sm:gap-5`}>
        {[
          { i: TrendingUp, t: "Earn 5% to 12%", d: "Tiered commission on every verified paid enrollment." },
      ...
      <section className={formGridClass}>
        ...
        <FormAside className="space-y-3 sm:space-y-5">
          <div className="rounded-2xl sm:rounded-3xl bg-gradient-to-br from-primary to-purple-900 text-primary-foreground p-5 sm:p-6 md:p-8">
            ...
            <p className="mt-4 font-display text-xl sm:text-2xl font-bold tracking-tight text-pretty">
            ...
            <ul className="mt-5 sm:mt-6 space-y-2.5 sm:space-y-3">
              {tiers.map((t) => (
                <li
                  key={t.range}
                  className="flex items-center justify-between gap-3 rounded-xl sm:rounded-2xl bg-white/10 px-3 sm:px-4 py-2.5 sm:py-3 border border-white/15"
                >
                  <span className="min-w-0 flex-1 text-sm font-medium leading-snug">{t.range}</span>
                  <span className="shrink-0 text-sm font-semibold text-right">{t.reward}</span>
                </li>
              ))}
            </ul>
            <p className="mt-4 sm:mt-5 text-xs sm:text-sm text-primary-foreground/80 leading-relaxed text-pretty">
              Commissions are paid only for verified fully paid enrollments. Cancelled or refunded
              enrollments do not qualify.
            </p>
          </div>

          <div className="rounded-2xl sm:rounded-3xl border border-border p-4 sm:p-6">
            <p className="font-display text-base sm:text-lg font-semibold">Referral process</p>
            <ol className="mt-3 sm:mt-4 space-y-2.5 sm:space-y-3 text-sm">
              {[
                ...
                <li key={s.t} className="flex items-start gap-3">
                  <span className="size-7 shrink-0 grid place-items-center rounded-lg bg-primary/10 text-primary text-xs font-semibold">
                    {idx + 1}
                  </span>
                  <span className="min-w-0 flex-1 pt-0.5 leading-relaxed text-pretty">{s.t}</span>
                </li>
              ))}
            </ol>
          </div>
          ...
          <div className="rounded-2xl sm:rounded-3xl border border-border p-4 sm:p-6">
            <p className="font-display text-base sm:text-lg font-semibold">Role of an ambassador</p>
          ...
          <div className="rounded-2xl sm:rounded-3xl border border-border p-4 sm:p-6">
            <p className="font-display text-base sm:text-lg font-semibold">Guidelines & policy</p>
            <ul className="mt-3 space-y-2 text-sm text-ink-muted leading-relaxed text-pretty">
