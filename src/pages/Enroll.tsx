...
import { uploadApplicationDocument } from "@/lib/api";
import {
  SuccessCard,
  SuccessDetail,
  SuccessParagraph,
...
export default function EnrollPage() {
    ...
    return (
      <SiteLayout>
        ...
        <div className="mx-auto max-w-2xl px-6 py-32 text-center">
          ...
          <Link to="/courses" className="mt-6 inline-block text-primary font-semibold">
            View all courses
          </Link>
        </div>
      </SiteLayout>
  ...
  return (
    <SiteLayout>
      <SEO
        title={`Enroll, ${course.title} | OxVerse Academy`}
        description={`Apply for ${course.title} at OxVerse Academy.`}
        noIndex
      />
      <section className="relative">
        ...
        <div className={`relative ${pageHeroClass} pb-6 sm:pb-8`}>
          <Link to={`/courses/${course.slug}`} className="text-sm text-ink-muted hover:text-ink">
            ← Back to {course.title}
          </Link>
          <h1 className="mt-4 font-display text-4xl md:text-5xl font-bold tracking-tighter">
            Reserve your seat
          </h1>
          <p className="mt-2 text-ink-muted">
            Apply for {course.title}, physical classes at our Lagos campus.
          </p>
          ...
          <div className="mt-8 flex items-center gap-1 overflow-x-auto pb-2">
            {STEPS.map((label, i) => (
              <div key={label} className="flex items-center gap-2 shrink-0">
                <span
                  className={`size-7 rounded-full inline-flex items-center justify-center text-xs font-semibold ${step >= i ? "bg-ink text-background" : "bg-muted text-ink-muted"}`}
                >
                  {step > i ? <Check className="size-4" /> : i + 1}
                </span>
                <span
                  className={`text-xs font-medium ${step >= i ? "text-ink" : "text-ink-muted"}`}
                >
                  {label}
                </span>
        ...
        >
              ...
              <Grid>
                ...
                <Field label="Gender">
                  <select
                    className="input"
                    value={form.gender}
                    onChange={(e) => set("gender", e.target.value as Application["gender"])}
                  >
                    {(["Male", "Female", "Prefer not to say"] as const).map((o) => (
              ...
              <Grid>
                <Field label="Current education level">
                  <select
                    ...
                    onChange={(e) =>
                      set("educationLevel", e.target.value as Application["educationLevel"])
                    }
                  >
                    {(
                      [
                ...
                <Field label="School / University" error={errors.school}>
                  <input
                    className="input"
                    value={form.school}
                ...
                <Field label="Employment status">
                  <select
                    ...
                    onChange={(e) =>
                      set("employmentStatus", e.target.value as Application["employmentStatus"])
                    }
                  >
                    {(["Student", "Employed", "Self employed", "Unemployed", "Other"] as const).map(
                      (o) => (
                        <option key={o}>{o}</option>
                      ),
                    )}
                ...
                <Field label="Current skill level">
                  <select
                    ...
                    onChange={(e) => set("skillLevel", e.target.value as Application["skillLevel"])}
                  >
                    {(["None", "Beginner", "Intermediate", "Advanced"] as const).map((o) => (
                      <option key={o}>{o}</option>
                    ))}
                  </select>
                </Field>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-1 text-sm text-ink-muted flex items-center gap-1">
                        ...
                        {full ? "Waitlist only" : `${i.seatsLeft} seats left`}
                      </p>
                    </button>
                  );
                })}
            ...
            <Card title="Documents (optional)" desc="You can also bring these on orientation day.">
              <div className="grid sm:grid-cols-2 gap-4">
                <UploadField
                  ...
                  onChange={(file) => {
                    set("passportPhotoName", file?.name ?? "");
                    set("passportPhotoFile", file);
                  }}
                  accept="image/*"
                />
                <UploadField
                  ...
                  onChange={(file) => {
                    ...
                    set("idDocumentFile", file);
                  }}
                  accept="image/*,application/pdf"
                />
              </div>
              <p className="mt-4 text-xs text-ink-muted">
                Images and PDFs up to 10MB are stored privately with your application.
              </p>
          ...
          {step === 5 && (
            <Card title="Review and submit" desc="Double check everything before sending.">
              ...
              {remoteError && <p className="mt-4 text-sm text-red-600">{remoteError}</p>}
              <div className="mt-8 flex justify-between">
                <button
                  onClick={() => setStep(4)}
                  className="rounded-full border border-border px-5 py-3 font-medium hover:bg-muted"
                >
                  Back
                </button>
                <button
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}>
              ...
              <SuccessActionGrid>
                <NextStep
                  ...
                  body="Admissions emails you within 48 hours with a confirmation and payment plan options."
                />
                <NextStep
                  title="Orientation"
                  body={`On ${intake.label} at 9:00am, No 82, Century Bus Stop, Ago Palace Way, Okota, Lagos.`}
                />
                <NextStep
                  title="Join the community"
                  body="Connect with your cohort on WhatsApp before classes start."
                  cta={{ label: "Join WhatsApp group", href: "https://chat.whatsapp.com" }}
                />
                <NextStep
                  title="Student dashboard"
                  body="Track your application and complete onboarding tasks."
                  cta={{ label: "Go to dashboard", to: `/dashboard?id=${appId}` }}
                />
              </SuccessActionGrid>
              <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row gap-3">
                <button
                  onClick={() => navigate(`/dashboard?id=${appId}`)}
                  className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-full bg-ink text-background px-6 py-3 font-semibold hover:bg-primary transition"
                >
                  Open student dashboard <ArrowRight className="size-4" />
                </button>
                <Link
                  to="/applications"
                  className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-full border border-border px-6 py-3 font-semibold hover:border-ink transition"
                >
          ...
          <div className="rounded-3xl border border-border bg-background overflow-hidden shadow-xl shadow-primary/5">
            ...
            <div className="p-6">
              ...
              <div className="mt-5 pt-5 border-t border-border space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-ink-muted">Intake</span>
                  <span className="font-medium">{intake.label}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ink-muted">Format</span>
                  <span className="font-medium">In person, Lagos</span>
                </div>
              </div>
            </div>
          </div>
...
function Nav({ onBack, onNext }: { onBack?: () => void; onNext: () => void }) {
  return (
    <div className="mt-8 flex justify-between">
      ...
      <button
        onClick={onNext}
        className="inline-flex items-center gap-2 rounded-full bg-ink text-background px-6 py-3 font-semibold hover:bg-primary transition"
      >
